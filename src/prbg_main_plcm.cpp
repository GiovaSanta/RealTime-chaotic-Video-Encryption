...
std::vector<double> generatePRBGMainKeys(double x0, double p, int numParameters4subsequentPRBGas, uint64_t *sc) {
    ...
    //printf("numParameters4subsequentPRBGas: %d\n",numParameters4subsequentPRBGas);
    for ( int i = 0 ; i < numParameters4subsequentPRBGas ; i++ ) {
        ...
        if( i < numParameters4subsequentPRBGas - 1 ){
            ...
            */
            ...
            //*sc = xi ;
            //*sc = static_cast<uint64_t>(xi * 10000000000000) ;  // when i was using double inside the confusion kernel operations  sc of magnitude up to 1000000000 yielded correct confusion output. when using float instead this limit is smaller
...
std::vector<double> generatePRBGMainKeysv2( double x0, double p, int n, uint64_t *sc ) {
    ...
    out.reserve(n - 1);  // avoid reallocations

    double xi = x0;

    // precompute inverses (faster than division)
...
/*std::vector<double> generatePRBGMainKeys_2DLASM(double x0, double mu, int numParameters4subsequentPRBGas, uint64_t *sc) {
