...
int main () {
    ...
    std::vector<uint8_t> output1_bytes, output2_bytes ;
    ...
    PRBGAKernelWrapper(keysAndControlPs, output1_bytes, output2_bytes, sc, PRBGAiterations);
    ...
    for (int i = 0; i < 128; i++) {
        ...
        for( int j = 0; j < 768 *6 ; j++){
            std::cout << std::hex << (int)output1_bytes[i*(768*6) + j] << " ";
        } 
        std::cout << "\n\nstring2  bytes: " ;
        for( int j = 0; j < 768 *6 ; j++){
            std::cout << std::hex << (int)output2_bytes[i*(768*6) + j] << " ";
        } 
    }
    std::cout << std::endl;
