...
int main(int argc, char** argv) {
    ...
    // ---- you assume this is always true, but we keep guard ----
...
void encrypt_thread() {
    ...
    // CUDA stream + events (encrypted completion events)
    ...
    // -------------------- DUMP FLAGS (first frame) --------------------
    bool dumped_plain = false;
    bool dumped_enc   = false;
    bool dumped_dec   = false;
    cv::Mat first_plain;
    ...
    while (true) {
        ...
        // ---- dump plaintext first frame ----
        if (pkt.id == 0 && !dumped_plain) {
            first_plain = pkt.frame.clone();
            cv::imwrite("testResults/first_plain.png", first_plain);
            std::cout << "\n[dump] wrote testResults/first_plain.png\n";
            dumped_plain = true;
        }
        ...
        // event recorded AFTER D2H(enc) so it gates correctness of h_enc_pinned[cur]
        CHECK_CUDA(cudaEventRecord(encStop[cur], stream1), "cudaEventRecord(encStop)");

        // ---- dump encrypted first frame (after 5 rounds) ----
        if (pkt.id == 0 && !dumped_enc) {
            CHECK_CUDA(cudaEventSynchronize(encStop[cur]), "cudaEventSync(first encStop)");
            cv::Mat enc0(height, width, CV_8UC3);
            std::memcpy(enc0.data, h_enc_pinned[cur], frameBytes);
            cv::imwrite("testResults/first_encrypted.png", enc0);
            std::cout << "[dump] wrote testResults/first_encrypted.png\n";
            dumped_enc = true;
        }
        ...
        // IMPORTANT: ensure D2H(dec) is finished before reading h_dec_pinned[cur]
        CHECK_CUDA(cudaStreamSynchronize(stream1), "cudaStreamSynchronize(after D2H(dec))");

        // ---- dump decrypted first frame ----
        if (pkt.id == 0 && !dumped_dec) {
            cv::Mat dec0(height, width, CV_8UC3);
            std::memcpy(dec0.data, h_dec_pinned[cur], frameBytes);
            cv::imwrite("testResults/first_decrypted.png", dec0);
            std::cout << "[dump] wrote testResults/first_decrypted.png\n";

            if (!first_plain.empty()) {
                double maxDiff = cv::norm(first_plain, dec0, cv::NORM_INF);
                std::cout << "[dump] max |plain - dec| = " << maxDiff << "\n";
            }
            dumped_dec = true;
        }
